...
pub fn msa1matrix(id: &str) -> Result<()> {
    ...
    for result in rdr.records() {
        ...
        row.push(record.get(3).unwrap_or("0.0").to_string());
        row.push(record.get(4).unwrap_or("0.0").to_string());
        x_matrix.push(row);
    }
    ...
    println!("\x1b[1;32mProcessing MSA1 positions...\x1b[0m");
...
pub fn compute_difference(
    ...
    for i in 0..num_leaves {
        if ps < msa_upd[i].len() {
            ...
            if num == 1 { all_prob[i][0] = 1.0; }
            else if num == 2 { all_prob[i][1] = 1.0; }
        }
    }
    ...
    for level_grp in levels.iter() {
        for pair_str in level_grp {
            ...
            let rg: usize = parts[0].parse().unwrap_or(0);
            let lf: usize = parts[1].parse().unwrap_or(0);
